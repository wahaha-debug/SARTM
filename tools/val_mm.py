...
def evaluate(model, dataloader, device):
    ...
    for images, labels in tqdm(dataloader):
        ...
        if sliding:
            ...
            m_output, output, prototype_loss, kl_loss = model(images, labels, multimask_output=True)

            preds = m_output.softmax(dim=1)
...
def main(cfg):
    ...
    eval_cfg = cfg['EVAL']
    transform = get_val_augmentation(eval_cfg['IMAGE_SIZE'], seg_fill=dataset_cfg['IGNORE_LABEL'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='/home/wahaha/zuizhong/configs/pst_rgbt.yaml')
